...
export default function Register() {
  ...
  function register() {
    NetworkService.postRegister(username, password).then(() => {
      ...
      enqueueSnackbar("Failed to register: " + err.response.data.message, { variant: "error" });
    });
  }
  ...
  return (
    <>
      ...
      /><br />
      {password.length > 0 &&
        <Typography variant="body2" color={theme.palette.error.dark} sx={{ marginBottom: "12px "}}>{validatePassword()[1]}</Typography>
      }
      <Button
        variant="contained"
        sx={{ marginBottom: "12px "}}
